...
declare -A linux_term_schemes
linux_term_schemes["konsole"]="$HOME/.local/share/konsole"
linux_term_schemes["xfce4terminal/colorschemes"]="$HOME/.local/share/xfce4/terminal/colorschemes"

# For iTerm schemes only
if [[ "$(uname -s)" != "Darwin" ]]; then
  if [[ "$(uname -s)" == "Linux" ]]; then
    failures=0
    for term in "${!linux_term_schemes[@]}"; do 
      if [ -d "${linux_term_schemes[${term}]}" ]; then 
        if ! cp -r "${term}"/* "${linux_term_schemes[${term}]}"; then 
          >&2 printf "failed to copy schems for ${term}"
          ((failures++))
        else
          echo "installed for ${term}"
        fi
      fi
    done
    exit ${failures}
